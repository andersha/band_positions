<script lang="ts">
  ...
  let { pieces = [], bandType = 'wind', sortOrder = 'asc', onRemove }: Props = $props();
  ...
  function ariaSort(current: PieceSortColumn, target: PieceSortColumn, dir: Direction): 'ascending' | 'descending' | 'none' {
    if (current !== target) return 'none';
    return dir === 'asc' ? 'ascending' : 'descending';
  }

  function indicator(current: PieceSortColumn, target: PieceSortColumn, dir: Direction): string {
    if (current !== target) return '';
    return dir === 'asc' ? ' ▲' : ' ▼';
  }

  function handleSort(column: PieceSortColumn) {
  ...
  function getValue(performance: PiecePerformance, column: PieceSortColumn) {
    switch (column) {
      case 'year': return performance.entry.year;
      case 'division': return getDivisionRank(performance.entry.division ?? null);
      case 'band': return performance.band ?? null;
      case 'rank': return performance.entry.rank ?? performance.entry.absolute_position ?? null;
      case 'points': return performance.entry.points ?? null;
      case 'conductor': return performance.entry.conductor ?? null;
    }
  }
  ...
  function sortPerformances(performances: PiecePerformance[]): PiecePerformance[] {
    ...
    sorted.sort((a, b) => {
      ...
      return cmp(a.entry.year, b.entry.year, 'asc');
    });
    return sorted;
  }
      ...
      <header class="piece-header">
        ...
        {#if onRemove}
          <button
            ...
            aria-label={`Fjern ${piece.name}`}
            onclick={() => onRemove!(piece.slug)}
          >×</button>
        {/if}
      </header>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th scope="col" class="sortable" onclick={() => handleSort('division')} aria-sort={ariaSort(sortColumn, 'division', sortDirection)}>Divisjon<span class="sort-indicator">{indicator(sortColumn, 'division', sortDirection)}</span></th>
                    ...
                    <div class="streaming-links">
                      ...
                      {#if streaming?.youtube}
                        <a
                          ...
                          title={`${piece.name} (YouTube)`}
                        >
                          <span class="sr-only">Hør på YouTube</span>
                          <svg class="streaming-icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false">
                            <circle cx="12" cy="12" r="10.5" opacity="0.15" fill="currentColor" />
                            <path
...
<style>
  ...
  @media (max-width: 640px) {
    ...
    /* Reorder cells: År/Divisjon, Korps/Plass, Dirigent/Poeng (swapped), Opptak */
    td[data-label="År"] { order: 1; }
    td[data-label="Divisjon"] { order: 2; }
    td[data-label="Korps"] { order: 3; }
    td[data-label="Plass"] { order: 4; }
