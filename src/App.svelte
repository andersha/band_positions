...
import type {
  ...
  let showStartupScreen = $state(false); // New state for startup screen
  ...
  function resolveInitialTheme(): Theme {
    ...
    const stored = readLS(STORAGE_KEYS.THEME, 'dark');
  ...
  async function applyThemePreference(nextTheme: Theme): Promise<void> {
    ...
    // Update status bar style for iOS (await to ensure it completes)
    await updateStatusBarStyle(nextTheme);
  }
  
  async function updateStatusBarStyle(currentTheme: Theme): Promise<void> {
    if (typeof document === 'undefined') return;
    
    const bgColor = currentTheme === 'light' ? '#f8fafc' : '#0f172a';
    
    // Update the meta tag for web/fallback
    ...
    // Use 'default' for light mode (dark text) and 'black-translucent' for dark mode (light text)
    ...
    // Update EdgeToEdge background color for Android
    try {
      await EdgeToEdge.setBackgroundColor({ color: bgColor });
    } catch (err) {
      // Plugin not available (iOS/web)
      console.log('EdgeToEdge plugin not available:', err);
    }
    
    // Use Capacitor Status Bar plugin for native control
    try {
      if (currentTheme === 'light') {
        await StatusBar.setStyle({ style: Style.Light }); // Light = dark text on light background
        await StatusBar.setBackgroundColor({ color: bgColor });
      } else {
        await StatusBar.setStyle({ style: Style.Dark }); // Dark = light text on dark background
        await StatusBar.setBackgroundColor({ color: bgColor });
      }
    } catch (err) {
      // Not running in native app or Status Bar plugin not available
  ...
  function resolveInitialBandType(): BandType | null {
    if (typeof window === 'undefined') return DEFAULT_BAND_TYPE;
    
    // First priority: URL parameter (deep link)
    const params = new URLSearchParams(window.location.search);
    ...
    // Check if there are any other URL parameters (view, selections, etc.)
    // If yes, use localStorage or default. If no, show startup screen.
    const hasOtherParams = params.has(URL_VIEW_KEY) || 
                          params.has('band') || 
                          params.has('conductor') || 
                          params.has('piece') || 
                          params.has('composer') ||
                          params.has(URL_YEAR_KEY) ||
                          params.has(URL_DIVISION_KEY);
    
    if (hasOtherParams) {
      // Deep link without band type - use localStorage or default
      const stored = readLS(STORAGE_KEYS.BAND_TYPE, DEFAULT_BAND_TYPE);
    ...
    // No URL parameters at all - show startup screen
    return null;
  }
  ...
  function setBandType(nextBandType: BandType, updateUrl: boolean = true): void {
    ...
    if (updateUrl && typeof window !== 'undefined') {
      ...
      params.set(URL_VIEW_KEY, DEFAULT_VIEW); // Set default view when selecting band type
  ...
  function toggleBandType(): void {
    if (bandType === null) return; // Safety check
  ...
  function handleStartupSelection(selectedType: BandType): void {
    setBandType(selectedType, true);
  }
  ...
  function setYAxisMode(mode: 'absolute' | 'relative'): void {
    ...
    yAxisMode = mode;
    writeLS(STORAGE_KEYS.YAXIS_MODE, mode);
  }
  ...
  function handleSettingsYAxisModeChange(event: CustomEvent<{ value: 'absolute' | 'relative' }>): void {
    setYAxisMode(event.detail.value);
  }

  function handleSettingsYAxisScaleChange(event: CustomEvent<{ value: 'fitted' | 'full' }>): void {
    setYAxisScale(event.detail.value);
  }

  function handleSettingsThemeChange(event: CustomEvent<{ value: 'light' | 'dark' }>): void {
  ...
  onMount(() => {
    // Prevent overscroll on iOS while allowing full content scrolling
    let startY = 0;
    ...
    const preventOverscroll = (e: TouchEvent) => {
      ...
      const scrollable = target.closest('[data-scrollable]') || 
                        ...
                        target.closest('.mode-toggle__menu');
  ...
  let leadText = $derived((() => {
    switch (activeView) {
      case 'bands':
        return bandType === 'wind'
      ...
      case 'conductors':
        return 'Søk etter en dirigent for å se hvordan deres beste plassering utvikler seg år for år, basert på korpsene de dirigerte.';
      case 'pieces':
        return 'Søk etter et musikkstykke for å se alle fremføringer vi har registrert.';
      case 'composers':
      ...
      case 'om':
        return '';
      case 'data':
        return 'Velg et år og en divisjon for å vise resultatlisten i den valgte finalen.';
      case 'innstillinger':
        return '';
      default:
        return '';
...
{#if showStartupScreen}
  <StartupScreen onSelect={handleStartupSelection} />
{:else}
<main>
  <header class="page-header">
    ...
    >
      ...
      <button
        ...
        class:band-type-toggle--to-brass={bandType === 'wind'}
        class:band-type-toggle--to-wind={bandType === 'brass'}
        type="button"
...
<style>
  ...
  :global(body) {
    /* Prevent overscroll bounce but allow normal scrolling */
    overscroll-behavior-y: none;
    min-height: 100%;
    height: auto;
  }
  ...
  :global(#app) {
    /* Make app scrollable with proper bounds */
    min-height: 100%;
    display: flex;
  ...
  /* Band type toggle: hint at brass vs wind */
  .band-type-toggle--to-brass {
    /* Going to brass - golden/warm tones */
    border-color: rgba(234, 179, 8, 0.3);
    background: rgba(234, 179, 8, 0.08);
  }

  .band-type-toggle--to-wind {
    /* Going to wind - cool/silver tones */
  ...
  @media (max-width: 780px) {
    ...
    /* Show full text for settings button in mobile menu */
    .header-controls .view-toggle__settings::before {
  ...
  /* iPhone-specific adjustments (all iPhones including 16 Pro Max) */
  @media (max-width: 450px) and (orientation: portrait) {
    h1 {
      font-size: 1.4rem;     /* Reduced from 1.6rem to prevent clipping */
      line-height: 1.25;
    }
    
    .page-header {
      margin-top: 0.5rem;    /* Pushes header down slightly */
    }
    
    .menu-toggle {
      margin-top: 0.25rem;   /* Extra clearance for tap area */
    }
  ...
  /* iPad-specific optimizations (landscape especially) */
